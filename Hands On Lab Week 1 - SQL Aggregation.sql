...
# 2. What is the average altitude for airports grouped by timezone.
# Which timezone has the highest altitude? Why?
# tz = -7 has the max AVG(alt); this is where the Rocky Mountains are.
SELECT tz,AVG(alt)
FROM airports
GROUP BY tz
ORDER BY 2 DESC;
...
SELECT tailnum, COUNT(*),origin
FROM flights WHERE tailnum IN ('N125UW','N848MQ','N328AA','N247JB')
GROUP BY tailnum
ORDER BY COUNT(*) DESC;

# 4. For each of these four planes, show the corresponding meta-data (model, manufacturer, engines, etc.)
# about each plane? What is surprising about the information returned? How do you think this could happen?
SELECT *
FROM planes
WHERE tailnum IN ('N125UW','N848MQ','N328AA','N247JB');

# 5. Write a SELECT statement that shows for all of the flights during the period February 14th to February 17th for each
